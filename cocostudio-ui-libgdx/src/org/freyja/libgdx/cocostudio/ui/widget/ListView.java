...
public class ListView extends Table implements Cullable {
	...
	public void setItems(Object[] objects, Class<?> cell, int itemWidth, int itemHeight) {
		if (objects == null)
		...
		for (int i = 0; i < items.length; i++) {
			try {
				BaseListCell t = (BaseListCell) cell.newInstance();
				t.setIndex(i);
				t.setParams(params);
				t.setCellSize(itemWidth, itemHeight);
				t.setData(items[i]);
